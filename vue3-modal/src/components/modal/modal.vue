<template>
    <div :style="styleObj">
        ...
        <div :class="['ant-modal-wrap',props.wrapClassName]" :style="{'z-index':props.zIndex}" tabindex="-1" aria-labelledby="vcDialogTitle0" @click.self="closeModal"> 
            <div :class="['ant-modal',props.centered && 'ant-modal-centered',dialogClass]" :style="[{width:props.width+'px'},props.dialogStyle]">             
                <div class="ant-modal-content">                   
                    ...
                    <div class="ant-modal-footer">
                        <slot name="footer" v-if="props.footer ==null ? false :$slots.footer ? true :false"></slot>
                        <div v-else>
                           <template v-if="props.footer==null ? false : true">
                                <button class="ant-btn ant-btn-close" @click="closeModal" :style="cancelButtonStyle">
                                    <!----><span>{{ props.cancelText }}</span>
                                </button>
                                <button class="ant-btn ant-btn-primary" :style="okButtonStyle" @click="okModal"><span>{{ props.okText }}</span>
                                </button>
                           </template>
                        </div>
                    </div>
...
const emit = defineEmits(['ok','visible','cancel']);
